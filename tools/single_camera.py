...
class SingleCamera:
    ...
    def __init__(self, position: List[float], clipping_range: List[float], resolution: List[int]) -> None:
        """
        Init.
    ...
    def get_cameras(self) -> Generator[Tuple[List[Any], List[Any]], None, None]:
        ...
        camera = rep.create.camera(position=self.position, clipping_range=tuple(self.clipping_range))
        render_product = rep.create.render_product(camera, self.cam_resolution)
        yield [camera], [render_product]
